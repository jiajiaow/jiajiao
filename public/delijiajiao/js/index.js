...
var base64encoder = __webpack_require__(50);
...
if (!WebSocket && typeof window === 'undefined') {
  try {
    WebSocket = __webpack_require__(79);
  } catch (e) { }
}
...
exports['default'] = extractInfo;
/**
 * this module is used to extract info from ad DOM element in desktop mode.
 * currently only support table/item view. waterfall view is not supported.
 * to extract info for mobile, use assets/mobile/js/ad-tracker/modules/extractAdInfo.js
 */
var $window = $(window);
var $body = $('body');
var viewType = void 0;
...
function extractInfo(ele) {
    ...
    if (viewType === 'item' || viewType === 'table') {
        ...
        for (var i = 0; i < serviceType.length; i++) {
            var _serviceNum = serviceType[i]['serviceNum'];
            var _selector = serviceType[i]['selector'];

            // 直接find付费服务的tag的选择器样式
            if ($ele.find(_selector).length <= 0) continue;
            ...
            if (_serviceNum !== 1001) {
                ...
                if (!serviceCollectionCache[_serviceNum]) {
                    serviceCollectionCache[_serviceNum] = $(containerSelector + " li[data-aid]").has(_selector);
                }
                payRank = serviceCollectionCache[_serviceNum].index($ele);
                ...
                if (!serviceCollectionCache[_serviceNum]) {
                    serviceCollectionCache[_serviceNum] = $(containerSelector + " li[data-aid].item-pinned").has(_selector);
                }
                payRank = serviceCollectionCache[_serviceNum].index($ele);
    ...
    if (aid) {
        adInfo = ['haojing', 'web', viewType, window.city, window.category, rank, payType, getServiceName(payType), payRank, window.location.href, $ele.data('aid'), +new Date()].join(',');

        // console.debug(adInfo);
    }
...
var _socket = __webpack_require__(67);

var _socket2 = _interopRequireDefault(_socket);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { 'default': obj }; }

// use websocket as default
var _method = 'websocket';

function setMethod(m) {
    if (m !== 'websocket' && m !== 'http') {
        throw new Error('no such method');
    }
    _method = m;

    if (m === 'websocket' && _timer) {
...
var _host = "http://d.admx.baixing.c" + "om:8882";
...
function setPlatform(p) {
    ...
    _platform = p;
    if (p === 'mobile') {
        _host = "http://d.admx.baixing.c" + "om:8882";
    } else if (p === 'desktop') {
        _host = "http://d.admx.baixing.c" + "om:8881";
    }
}

function send() {
    if (_method === 'websocket') {
        sendWebsocket.apply(null, arguments);
    } else if (_method === 'http') {
        sendHTTP.apply(null, arguments);
    }
}
...
var INTERVAL = 1500;
var _failedTimes = 0;
var FAIL_TORRENCE = 10;
/**
 * send data in normal http
 *
 * @param {String} endpoints api path
 * @param {String} data      data to send
 *
 */
function sendHTTP(endpoints, data) {
    // note: set contentType: text/plain will not issue a OPTIONS request.
    if (endpoints === 'ad-click') {
        $.ajax({
            url: _host + '/' + endpoints,
            method: "POST",
            ...
            window.tracker.count('ctr_logging', _platform === 'mobile' ? 'mobile.http_ok' : 'desktop.http_ok');
        }).fail(function () {
            window.tracker.count('ctr_logging', _platform === 'mobile' ? 'mobile.http_fail' : 'desktop.http_fail');
        });
        return;
    }
    ...
    if (!_timer) {
        _timer = setInterval(function () {
            if (_failedTimes >= FAIL_TORRENCE) {
                // console.debug('http failed after ' + FAIL_TORRENCE + ' trials');
                clearInterval(_timer);
                return;
            }

            var length = _dataPool.length;
            if (length) {
                $.ajax({
                    url: _host + '/' + endpoints,
                    method: "POST",
                    data: JSON.stringify(_dataPool),
                    contentType: "text/plain"
                }).done(function () {
                    _failedTimes = 0;
                    _dataPool.splice(0, length);
                    window.tracker.count('ctr_logging', _platform === 'mobile' ? 'mobile.http_ok' : 'desktop.http_ok');
                }).fail(function () {
                    _failedTimes++;
                    window.tracker.count('ctr_logging', _platform === 'mobile' ? 'mobile.http_fail' : 'desktop.http_fail');
...
// 0, 1, 2 = 'not started', 'connecting', 'connected';
// before ws got connected, cache data.
var _connectStatus = 0;
// const _host = "http://localhost:3001";
var _connection = null;

var _dataBeforeConnection = [];

function sendWebsocket(endpoints, data) {
    if (_connectStatus === 0) {
        initWebsocket(_host, function (socket) {
            if (_dataBeforeConnection.length) {
                $.each(_dataBeforeConnection, function (i, d) {
                    socket.emit(endpoints, d);
                });
            }
            socket.emit(endpoints, data);
        });
    } else if (_connectStatus === 1) {
        _dataBeforeConnection.push(data);
    } else if (_connectStatus === 2) {
        _connection.emit(endpoints, data);
    }
}

function initWebsocket(host, callback) {
    _connection = (0, _socket2['default'])(host, {
    ...
    _connection.on('connect', function () {
        ...
        window.tracker.count('ctr_logging', _platform === 'mobile' ? 'mobile.socket_connect' : 'desktop.socket_connect');
    });

    _connection.on('connect_error', function () {
        _connectStatus = 1;
        window.tracker.count('ctr_logging', _platform === 'mobile' ? 'mobile.socket_connect_error' : 'desktop.socket_connect_error');
    });

    _connection.on('reconnect', function () {
        window.tracker.count('ctr_logging', _platform === 'mobile' ? 'mobile.socket_reconnect' : 'desktop.socket_reconnect');
    });

    _connection.on('reconnect_attempt', function () {
        window.tracker.count('ctr_logging', _platform === 'mobile' ? 'mobile.socket_reconnect_attempt' : 'desktop.socket_reconnect_attempt');
    });

    _connection.on('error', function (e) {
        // console.debug(e);
        window.tracker.count('ctr_logging', _platform === 'mobile' ? 'mobile.socket_error' : 'desktop.socket_error');
    });

    _connection.on('reconnect_failed', function () {
        // console.debug('websocket failed, try http...');
        window.tracker.count('ctr_logging', _platform === 'mobile' ? 'mobile.socket_reconnect_fail' : 'desktop.socket_reconnect_fail');
        setMethod('http');
    });
}
...
exports.__esModule = true;
exports['default'] = trackAdClick;

var _transporter = __webpack_require__(83);

var _transporter2 = _interopRequireDefault(_transporter);

var _extractAdInfo = __webpack_require__(82);

var _extractAdInfo2 = _interopRequireDefault(_extractAdInfo);
...
if ($body.hasClass('view-item')) {
    containerSelector = '.list-ad-items';
} else if ($body.hasClass('view-table')) {
    containerSelector = '.table-view';
}

function trackAdClick() {
    _transporter2['default'].setPlatform('desktop');
    var $element = $(containerSelector + ' li[data-aid]');
    if ($element.length === 0) {
        // console.log('no elements to track')
        return;
    }

    $element.on('click', function (event) {
        if ($(event.target).hasClass('ad-title') || $(event.target).parent().hasClass('media-cap')) {
            var info = (0, _extractAdInfo2['default'])(event.currentTarget);
            if (info) {
                _transporter2['default'].send('ad-click', info);
            }
        }
    });
}
...
exports.__esModule = true;
exports['default'] = trackAdView;

var _viewability = __webpack_require__(185);

var _viewability2 = _interopRequireDefault(_viewability);
...
function trackAdView() {
    _transporter2['default'].setPlatform('desktop');
    _viewability2['default'].setSelectors([containerSelector + ' li[data-aid]']);
    _viewability2['default'].setHandler(function (ele) {
        var info = (0, _extractAdInfo2['default'])(ele);
        if (info) {
            _transporter2['default'].send('ad-view', info);
        }
    });
    _viewability2['default'].init();
    // console.debug('trackAdView ...');
}
...
var _trackAdView = __webpack_require__(93);

var _trackAdView2 = _interopRequireDefault(_trackAdView);

var _trackAdClick = __webpack_require__(92);

var _trackAdClick2 = _interopRequireDefault(_trackAdClick);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { 'default': obj }; }

(0, _trackAdView2['default'])();
(0, _trackAdClick2['default'])();
...
function checkConfig() {
    if (!$.isArray(config.adSelector)) {
        throw Error('adSelector should be an array');
    }
    if (config.adSelector.length === 0) {
...
function getAllAds(selectors) {
    ...
    $.each(selectors, function (index, selector) {
        ads = ads.concat($(selector).toArray());
    });
    return ads;
}
...
function isElementInViewport(el, percent) {
    if (document.hidden || isElementHidden(el)) {
        return false;
    }
    ...
    return rect.top + effectiveHeight >= 0 && rect.left + effectiveWidth >= 0 && rect.bottom - effectiveHeight <= winH && /*or $(window).height() */
    rect.right - effectiveWidth <= winW /*or $(window).width() */
    ;
}

function isElementHidden(ele) {
    return ele.style.display === 'none' || ele.style.opacity === "0" || ele.style.visibility === 'collapse' || ele.style.visibility === 'hidden';
}
...
var api = {
    init: init,
    setHandler: function setHandler(fn) {
        _extractInfoHandler = fn;
    },
    setSelectors: function setSelectors(arr) {
