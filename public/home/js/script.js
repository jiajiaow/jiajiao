(function($) {
	...
	function susStyle() {
		if($(".order-form").length){
			var windowpos = $(window).scrollTop(),
				_offtop = $("#content").offset().top,
				_maxofftop = $(".nav-page").offset().top - _offtop;
			if (_maxofftop > windowpos && windowpos >= _offtop) {
				$(".order-form").stop(true).delay(300).animate({top:  windowpos - _offtop- 35+ "px" },  { duration: 500, easing: "linear" });
			} else if (windowpos >= _maxofftop){
				$(".order-form").stop(true).delay(300).animate({top: _maxofftop - _offtop - 250+ "px" },  { duration: 500, easing: "linear" });		
			}else{
				$(".order-form").stop(true).delay(300).animate({top: -25 + "px" },  { duration: 500, easing: "linear" });
			}
		}
	...
	$("#bjtx").click(function(e) {
		...
		$("input.name").blur(function(){
			...
			if(testValue.test(nameValue))		
			{
				if (nameValue.length > 4 || nameValue.length <= 1 ) {
					pCont.show();
					pCont.html("姓名为2-4个字符！")
				}else {
					pCont.html("");
				}
			} else {
		...
		// 身份证
		$("input.sfz").blur(function(){
			var sfzLength = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/,
				sfzValue = $("input.sfz").val(),
				pCont = $(this).siblings("p");
				if(!sfzLength.test(sfzValue)){
					pCont.show();
					pCont.html("请输入正确的身份证号！");
				}else {
					pCont.html("");
				}
		})
		...
		$("input.phone").blur(function(){
			var telTrue = /^(((13[0-9]{1})|(14[0-9]{1})|(17[0]{1})|(15[0-3]{1})|(15[5-9]{1})|(18[0-9]{1}))+\d{8})$/,
				telValue = $("input.phone").val(),
				pCont = $(this).siblings("p");
				if (!telTrue.test(telValue)) {
					pCont.show();
					pCont.html("请输入正确的手机号！");
				}else {
					pCont.html("");
				}
		})
